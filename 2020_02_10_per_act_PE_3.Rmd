...
# geom_bar
ggplot(aud, aes(congruency, rt)) + geom_bar(aes(fill = as.factor(congruency)), position = "dodge", stat = "summary", fun.y = "mean") + geom_errorbar(stat = 'summary', fun.data = mean_se, width = 0.25) + labs(title = 'Hey', subtitle = 'whatup') + theme(legend.title = element_blank()) #+ scale_colour_hue(labels = c('congruent', 'incongruent')) 
